...
export default function CoursesPage() {
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      // First try to get persisted recommendations
      const response = await fetch('/api/user/recommendations', {
        method: 'GET'
      })
      ...
      if (response.ok) {
        ...
        if (persistedRecommendations.length > 0) {
          ...
          const formattedRecommendations = persistedRecommendations.map((rec: any, index: number) => ({
            ...
            description: rec.reasoning || 'AI-recommended course based on your profile',
            category: rec.category || 'AI Recommended',
            duration: rec.estimated_duration || 'Variable',
            level: rec.level,
            provider: rec.provider,
          ...
          // If no persisted recommendations, generate new ones
          const generateResponse = await fetch('/api/user/recommendations', {
            method: 'POST',
          ...
          if (generateResponse.ok) {
            ...
            const newRecommendations = generateData.recommendations || []
